'''
Only use the x-axis data to detect the information tag consisting of 0-degree magnet
'''
...
slope_list_x, raw_list_x = ([[] for _ in range(num)] for _ in range(2))
slope_thrd_x = [10000] * num
estimate_x = [True] * num
LastRAW_x = [0] * num  # Record the index of the last peak
N_flag_x, S_flag_x = ([0] * num for _ in range(2))
...
def detectMag(listFrames, listFrames2):
    ...
    arduino.flushInput()
    t = []
    global cnt, current, no_x, n_x, n, cnt_mag, cnt_exp, x, sx, dx, sdx
    global slope_thrd_x, slope_list_x, estimate_x, raw_result, raw_list_x
